...
export default async function HomePage() {
  ...
  const [transports, user] = await Promise.all([getTransportCards("", lang), getCurrentUser()]);
  ...
  return (
    <>
        ...
        <div className="container hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              ...
              <Link href={user ? "#transports" : "/signup?next=/%23transports"} className="button button-ghost">{t.shareLocation} <LocateFixed size={19} /></Link>
            </div>
          ...
          <div className="hero-map-card">
            <div className="mini-map">
              ...
              {liveVehicles > 0 && <div className="vehicle-marker v1"><span><LocateFixed size={16} /></span><div><strong>{liveVehicles} {t.vehiclesLive}</strong><small>{t.updatedByPassengers}</small></div></div>}
              {liveVehicles > 0 && <div className="pulse-ring" />}
            </div>
            <div className="map-card-footer">
              <div><span className="icon-tile"><MapPin size={20} /></span><p><strong>{liveVehicles} {t.vehiclesLive}</strong><small>{liveVehicles ? t.acrossActiveRoutes : t.awaitingUpdate}</small></p></div>
              {activePeople > 0 && <div className="people-live"><UsersRound size={17} /><span>{activePeople}</span></div>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="cta-band">
        <div className="container cta-inner"><div><span className="eyebrow light">{t.betterTogether}</span><h2>{t.onABusNow}</h2><p>{t.oneUpdateHelps}</p></div><Link href={user ? "/#transports" : "/signup?next=/%23transports"} className="button button-light">{t.startHelping} <ArrowRight size={19} /></Link></div>
      </section>
    </>
  );
}
