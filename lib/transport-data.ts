...
export async function getTransportCards(query = "", lang: Lang = "en") {
  ...
  return transports.map((transport) => ({
    ...
    nameBn: transport.nameBn || "",
    slug: transport.slug,
    imageUrl: transport.imageUrl,
    routeName: lang === "bn" && transport.routeNameBn ? transport.routeNameBn : transport.routeName,
    routeNameBn: transport.routeNameBn || "",
    routeStops: lang === "bn" && transport.routeStopsBn?.length ? transport.routeStopsBn : transport.routeStops,
    routeStopsBn: transport.routeStopsBn || [],
    stopCoords: (transport.stopCoords || []).map((stop: { name: string; nameBn?: string; lat: number; lng: number }) => ({ name: stop.name, nameBn: stop.nameBn || "", lat: stop.lat, lng: stop.lng })),
    searchText: [transport.name, transport.nameBn, transport.routeName, transport.routeNameBn, ...(transport.routeStops || []), ...(transport.routeStopsBn || [])].filter(Boolean).join(" "),
